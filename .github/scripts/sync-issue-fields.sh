...
# ====================================
# For each issue in Ontocratic template-compatible projects, reads Priority
# and Story Points from the issue body and syncs them to the project item's
# Priority, Size, and Estimate fields. Also ensures labels are consistent
# across all project items for the same issue.
#
# Source of truth: issue body (parsed from Metatrom idea template sections).
# All eligible projects the issue belongs to are updated.
#
# Template detection: same as sync-project-status.sh — only projects whose
# Status field contains Purpose, Intention, and Action participate.
...
# --- Step 1: Get all org projects with fields, items, and issue bodies ---
...
# --- Step 4: Group by content_id and sync fields to all project items ---
...
while IFS= read -r content_id; do
  ...
  # Apply to every project item for this issue
